...
function Retry() {
  ...
  async function getWalletPaymentHistory(walletAddress, paginationKey) {
  ...
  async function handleSignRetry(optionIndex) {
    ...
    try {
      ...
      // Sign the retry transfer transaction using wagmi
  ...
  // Effect: fetch erring payments when connected and assets loaded
  useEffect(() => {
    if (isConnected && userAddress && supportedAssets) {
      showErringPayments(userAddress, supportedAssets)
    }
  }, [isConnected, userAddress, supportedAssets])
  ...
  if (!sdkHasLoaded) {
    ...
    loginSection = (
      <p className="info-label">
        ...
        <br />
        <span className="address-label">{address || 'Loading...'}</span>
        <br />
        <button className="disconnect" onClick={handleLogOut}>Disconnect</button>
      </p>
    )
  }
